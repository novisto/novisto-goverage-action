...
async function checkForSkipComment(githubToken) {
  ...
  const number =
    process.env.OVERRIDE_PR_NUMBER ||
...
function formatComment(inputs, currentCoverage, files, diffCheck) {
  const emoji =
    currentCoverage < inputs.coverageThreshold ? FAILURE_EMOJI : SUCCESS_EMOJI;
  const adjective =
    currentCoverage < inputs.coverageThreshold ? "below" : "above";
...
async function postCommentOnPR(inputs, coverageJSON, diffCheck) {
  ...
  if (matchingComment !== undefined) {
    core.info(
      `Updating comment ${matchingComment.id} with new coverage details`,
    );
...
async function compareCoverageWithBaseRef(inputs, currentCoverage) {
  ...
  const baseRef =
    process.env.OVERRIDE_BASE_REF ||
    github.context.payload.pull_request.base.ref;
  const urlEncodedBaseRef = encodeURIComponent(baseRef);

  // call goverage to get the current coverage
  core.info(
    `Fetching coverage for ${inputs.projectName} on ${baseRef} from ${inputs.goverageHost}`,
  );
  ...
  if (response.status === 404) {
    core.warning(
      `Coverage not found for ${inputs.projectName} on ${baseRef} from ${inputs.goverageHost}`,
    );
    ...
    if (currentCoverage < baseCoverage - inputs.coverageDiffThreshold) {
      core.setFailed(
        `Coverage decreased by ${round(change, 2)}% compared to ${baseRef} which is more than the allowed threshold of ${inputs.coverageDiffThreshold}%`,
      );
      success = false;
    } else {
      core.info(
        `Coverage changed by ${round(change, 2)}% compared to ${baseRef}`,
      );
      success = true;
    }
  }
...
async function publishCoverage(inputs, coverage) {
  ...
  if (ref.startsWith(REFS_TAGS_PREFIX)) {
    refName = ref.substring(REFS_TAGS_PREFIX.length);
  ...
  if (response.status !== 201) {
    ...
    core.warning(
      `Failed to publish coverage to ${inputs.goverageHost}, status code: ${response.status}`,
    );
  }
}
...
function validateInputs(inputs) {
  ...
  if (inputs.goverageToken && !inputs.goverageHost) {
    throw new Error(
      "goverage_host must be provided to publish coverage, only got goverage_token",
    );
  }

  if (!inputs.goverageToken && inputs.goverageHost) {
    throw new Error(
      "goverage_token must be provided to publish coverage, only got goverage_host",
    );
  }

  if (
    inputs.publishCoverage &&
    (!inputs.goverageHost || !inputs.goverageToken)
  ) {
    core.warning(
  ...
  try {
    ...
    const inputs = {
      ...
      coverageThreshold: parseInt(
        core.getInput("coverage_threshold", { required: true }),
      ),
      coverageDiffThreshold: parseInt(
        core.getInput("coverage_diff_threshold", { required: false }),
      ),
      publishCoverage:
        core.getInput("publish_coverage", { required: true }) === "true",
    ...
    core.info(
      `Reading coverage file: ${inputs.projectPath}/${inputs.coverageFile}`,
    );
    const coverage = await fs.readFile(
      `${inputs.projectPath}/${inputs.coverageFile}`,
      "utf8",
    );
    ...
    if (github.context.eventName === "pull_request") {
      ...
      if (
        inputs.coverageDiffThreshold !== 0 &&
        inputs.goverageHost !== "" &&
        inputs.goverageToken !== ""
      ) {
